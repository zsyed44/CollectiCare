...
public class PatientRepository {
    ...
    /**
     * Utility method to fetch an active database session and bind it to the current
     * thread.
     */
    ...
    public Optional<Patient> getPatientByIdAndAddress(String patientID, String address) {
        try {
            ODatabaseSession dbSession = getDatabaseSessionWithThreadBinding();
            var resultSet = dbSession.query("SELECT FROM Patient WHERE PatientID = ? AND Address = ?", patientID,
                    address);
            if (resultSet.hasNext()) {
                var result = resultSet.next();
                if (result.getRecord().isPresent()) {
